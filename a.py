...
def get_parser():
    ...
    parser.add_argument('--input_path', type=str, default="./InputData.json", required=False,
...
if __name__ == '__main__':
    ...
    distance_matrix = [[0 for x in range(n_customers+1)] for y in range(n_customers+1)]
    for x in range(1, n_customers):
        key = "customer_" + str(x)
        x_coord = float(instance[key]["coordinates"]["x"])
        y_coord = float(instance[key]["coordinates"]["y"])

        for y in range(1, n_customers):
            x_coord2 = float(instance[key]["coordinates"]["x"])
            y_coord2 = float(instance[key]["coordinates"]["y"])
            distance_matrix[x][y] = euclideanDistance(x_coord, x_coord2, y_coord, y_coord2)
